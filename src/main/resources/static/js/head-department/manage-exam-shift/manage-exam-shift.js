$(document).ready(() => {
  ...
  $("#btnDownloadExamShiftTemplate").click(() => handleDownloadTemplate());
...
const handleUploadExamShiftExcel = () => {
  ...
  $.ajax({
    url: `${ApiConstant.API_HEAD_DEPARTMENT_EXAM_SHIFT}/file/upload`,
    type: "POST",
    data: formData,
    processData: false,
    contentType: false,
...
const handleDownloadTemplate = () => {
  window.open(
    `${ApiConstant.API_HEAD_DEPARTMENT_EXAM_SHIFT}/file/download`,
    "_blank",
  );
};
...
const getHistoryAssignSubject = () => {
  $.ajax({
    type: "GET",
    url: ApiConstant.API_HEAD_DEPARTMENT_EXAM_SHIFT + "/history",
    ...
    success: (responseBody) => {
      ...
      const historyAssignSubjects = responseBody?.data?.data?.map(
        (history, index) => {
          return `
                    <tr>
                        <td>${index + 1}</td>
    ...
    error: (error) => {
      showToastError(
        error?.responseJSON?.message || "Có lỗi xảy ra, vui lòng thử lại sau",
      );
    },
  });
};
...
const getListExamShift = () => {
  const url = getUrlParameters(
    `${ApiConstant.API_HEAD_DEPARTMENT_EXAM_SHIFT}`,
    examShiftParams,
  );
  $.ajax({
    url: url,
...
const handleFilter = () => {
  ...
  examShiftParams.examDate = $("#examDate").val()
    ? new Date($("#examDate").val()).getTime()
    : null;
  getListExamShift();
};
  ...
  $.ajax({
    ...
    success: (res) => {
      ...
      const blocks = data.map(
        (block) => `<option value="${block.id}">${block.blockInfo}</option>`,
      );
